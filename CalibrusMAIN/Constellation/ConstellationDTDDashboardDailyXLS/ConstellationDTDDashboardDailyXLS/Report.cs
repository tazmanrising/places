...
namespace ConstellationDTDDashboardDailyXLS
{
    public class Report
    {
        ...
        public static void Main(string[] args)
        {
            ...
            try
            {
                ...
                string sheetName = String.Format("{0}", "Constellation DTD Daily " + CurrentDate.ToString("MMM") + " " + CurrentDate.ToString("yyyy"));
        ...
        public static void WriteReport(ref Excel.Application exApp, ref Excel.Range exRange, DateTime currentDate, DateTime monthStartDate, DateTime yearStartDate)
        {
            ...
            foreach (Vendor vendor in vendorList)
            {
                ...
                foreach (Commodity fueltype in Enum.GetValues(typeof(Commodity)))
                {
                    ...
                    //LDC Code
                    List<string> LDCCodes = GetUDCCode(vendor.VendorNumber.ToString(), fueltype.ToString());
                    foreach (string ldccode in LDCCodes)
                    {
                        ...
                        yesterdayCount = GetUDCVerifiedCount(vendor.VendorNumber.ToString(), fueltype.ToString(), ldccode.ToString(), currentDate, "yesterday");
                        ...
                        while (dataDateCounter < dataEndDate)
                        {
                            if (dataCounter < 53)
                            {
                                ...
                                exRange = (Excel.Range)exApp.Cells[rowCount, ConvertColumn(colCount)];
                                exRange.Value2 = GetUDCVerifiedCount(vendor.VendorNumber.ToString(), fueltype.ToString(), ldccode.ToString(), GetFriday(dataDateCounter), "weekly");
                        ...
                        {
                            exRange = (Excel.Range)exApp.Cells[rowCount, ConvertColumn(colCount)];
                            exRange.Value2 = GetUDCVerifiedCount(vendor.VendorNumber.ToString(), fueltype.ToString(), ldccode.ToString(), dataDateCounter, "mtd");
                        ...
                        //YTD values
                        exRange = (Excel.Range)exApp.Cells[rowCount, ConvertColumn(colCount)];
                        exRange.Value2 = GetUDCVerifiedCount(vendor.VendorNumber.ToString(), fueltype.ToString(), ldccode.ToString(), currentDate, "ytd");
            ...
            exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
            exRange.Value2 = GetUDCVerifiedCount("Electric", currentDate, "yesterday", vendorList);
            exRange.Interior.ColorIndex = 15; //light grey
            exFont = exRange.Font;
            exFont.ColorIndex = 1;
            exFont.Bold = true;
            ...
            exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
            exRange.Value2 = GetUDCVerifiedCount("Gas", currentDate, "yesterday", vendorList);
            exRange.Interior.ColorIndex = 15; //light grey
            exFont = exRange.Font;
            exFont.ColorIndex = 1;
            exFont.Bold = true;
            ...
            exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
            exRange.Value2 = GetUDCVerifiedCount("Dual", currentDate, "yesterday", vendorList);
            exRange.Interior.ColorIndex = 15; //light grey
            exFont = exRange.Font;
            exFont.ColorIndex = 1;
            exFont.Bold = true;
            ...
            while (globalDateCounter < globalEndDate)
            {
                ...
                if (globalCounter < 53)
                {
                    ...
                    exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
                    exRange.Value2 = GetUDCVerifiedCount("Electric", GetFriday(globalDateCounter), "weekly", vendorList);
                    exRange.Interior.ColorIndex = 15; //light grey
                    exFont = exRange.Font;
                    exFont.ColorIndex = 1;
                    exFont.Bold = true;
                    ...
                    exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
                    exRange.Value2 = GetUDCVerifiedCount("Gas", GetFriday(globalDateCounter), "weekly", vendorList);
                    exRange.Interior.ColorIndex = 15; //light grey
                    exFont = exRange.Font;
                    exFont.ColorIndex = 1;
                    exFont.Bold = true;
                    ...
                    exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
                    exRange.Value2 = GetUDCVerifiedCount("Dual", GetFriday(globalDateCounter), "weekly", vendorList);
                    exRange.Interior.ColorIndex = 15; //light grey
                    exFont = exRange.Font;
                    exFont.ColorIndex = 1;
                    exFont.Bold = true;
            ...
            while (globalDateCounter < globalEndDate)
            {
                ...
                exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
                exRange.Value2 = GetUDCVerifiedCount("Electric", GetFriday(globalDateCounter), "mtd", vendorList);
                exRange.Interior.ColorIndex = 15; //light grey
                exFont = exRange.Font;
                exFont.ColorIndex = 1;
                exFont.Bold = true;
                ...
                exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
                exRange.Value2 = GetUDCVerifiedCount("Gas", GetFriday(globalDateCounter), "mtd", vendorList);
                exRange.Interior.ColorIndex = 15; //light grey
                exFont = exRange.Font;
                exFont.ColorIndex = 1;
                exFont.Bold = true;
                ...
                exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
                exRange.Value2 = GetUDCVerifiedCount("Dual", GetFriday(globalDateCounter), "mtd", vendorList);
                exRange.Interior.ColorIndex = 15; //light grey
                exFont = exRange.Font;
                exFont.ColorIndex = 1;
                exFont.Bold = true;
            ...
            exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
            exRange.Value2 = GetUDCVerifiedCount("Electric", currentDate, "ytd", vendorList);
            exRange.Interior.ColorIndex = 15; //light grey
            exFont = exRange.Font;
            exFont.ColorIndex = 1;
            exFont.Bold = true;
            ...
            exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
            exRange.Value2 = GetUDCVerifiedCount("Gas", currentDate, "ytd", vendorList);
            exRange.Interior.ColorIndex = 15; //light grey
            exFont = exRange.Font;
            exFont.ColorIndex = 1;
            exFont.Bold = true;
            ...
            exRange = (Excel.Range)exApp.Cells[globalRowCount, ConvertColumn(globalColCount)];
            exRange.Value2 = GetUDCVerifiedCount("Dual", currentDate, "ytd", vendorList);
            exRange.Interior.ColorIndex = 15; //light grey
            exFont = exRange.Font;
            exFont.ColorIndex = 1;
            exFont.Bold = true;
        ...
        /// </summary>
        /// <returns>All Vendors for SalesChannelId = 2 DTD</returns>
        private static List<Vendor> GetVendors()
        {
            //Filter for VendorIds to not use
            var myInClause = new int[] { 2, 7, 8, 18 };//Vendor 1, Test1, Vendor 2, Archived Agents

            List<Vendor> vendors = new List<Vendor>();
            using (ConstellationEntities data = new ConstellationEntities())
            {

                vendors = data.Vendors.Where(v => v.SalesChannelId == 1 && !myInClause.Contains(v.VendorId)).ToList();

                //TEST USING ONLY NATIONAL 1
                // vendors = data.Vendors.Where(v => v.SalesChannelId == 1 && v.VendorId == 17).ToList();
            }
            return vendors;
        }
        ...
        /// <summary>
        /// Get the Global counts for the  specific commodity, date range and the date type passed in with list of Vendors
        /// </summary>
        /// <param name="vendorId"></param>
        /// <param name="signupType"></param>
        /// <param name="udccode"></param>
        ...
        /// <returns></returns>
        private static int GetUDCVerifiedCount(string signupType, DateTime baseDate, string dateType, List<Vendor> vendors)
        {
            int count = 0;

            List<string> VendorNumbers = vendors.Select(v => v.VendorNumber).ToList();
                ...
                switch (signupType)
                {
                    case "Electric":
                        query = from m in data.tblMains
                                ...
                                && m.CallDateTime > startDate
                                && m.CallDateTime < endDate
                                && VendorNumbers.Contains(m.VendorId)
                                select m;
        ...
        {
            //Build the file name
            xlsFilename = "ConstellationDTDDashboardDailyReport" + String.Format("{0:yyyyMMdd}", cDate) + ".xlsx";
        ...
        private static void SendEmail(ref string xlsFilePath, DateTime currentDate, String strToEmail)
        {
            ...
            try
            {
                ...
                mail.Subject = "Constellation DTD Dashboard Daily Report for " + currentDate.ToString("MMM") + " " + currentDate.ToString("dd") + " " + currentDate.ToString("yyyy") + ".";
        ...
        {
            Calibrus.ErrorHandler.Alerting alert = new Calibrus.ErrorHandler.Alerting("ConstellationDTDDashboardDailyReport");
