...
namespace SparkProviderBatchXLSReport
{
    public class Report
    {
        ...
        public static void Main(string[] args)
        {
            ...
            List<BrandObject> brandList = GetBrands();
            ...
            //Loop through Brands, then Vendor then Utility then PremiseTypes (5 nested loops)
            #region BrandObject Loop
                    ...
                    foreach (UtilityObject utility in utilityList)
                    {
                        ...
                        foreach (UtilityTypeObject utilitytype in utilityTypeList)
                        {
                            ...
                            foreach (PremiseTypeObject premise in premiseTypeList)
                            {
                                ...
                                List<Record> recordList = GetListOfRecords(StartDate, EndDate, premise.PremiseTypeId, utility.UtilityId, utilitytype.UtilityTypeId, vendor.SalesChannelId, vendor.VendorId, brand);
                                ...
                                if (recordList.Count() > 0)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        SaveXlsDocument(ref rootPath, ref xlsFilename, ref xlsFilePath, exBook, StartDate, premise.PremiseTypeName, utility.LdcCode, utilitytype.UtilityTypeName, vendor.SalesChannelName, vendor.VendorName, brand);
                                    }
                                    catch (Exception ex)
                                    {
                                        SendErrorMessage(ex);
                                    ...
                                    //FTP report
                                    FTPFile(ref rootPath, ref xlsFilename, ref xlsFilePath, StartDate, premise.PremiseTypeName, utility.LdcCode, utilitytype.UtilityTypeName, vendor.SalesChannelName, vendor.VendorName, hostName, userName, password, brand);
                                }
                            }
            ...
            #endregion BrandObject Loop
        }

        private static List<BrandObject> GetBrands()
        {
            //2 Oasis data, 5 Provider data, 7 ENH Power, 8  Main Electricity, 9 Provider Power Mass    
            List<BrandObject> brandlist = new List<BrandObject>();
            BrandObject brand = new BrandObject();
            //Oasis
            brand.Name = "Oasis";
            BrandObjectID brandid = new BrandObjectID();
            brandid.id = 2;
            List<BrandObjectID> idList = new List<BrandObjectID>();
            idList.Add(brandid);
            ...
            //cleanup
            brand = new BrandObject();
            idList = new List<BrandObjectID>();

            //Provider
            brand.Name = "Provider";
            brandid = new BrandObjectID();
            brandid.id = 7;
            idList.Add(brandid);
            brandid = new BrandObjectID();
            brandid.id = 8;
        ...
        private static void WriteReport(ref Excel.Application exApp, ref Excel.Range exRange, DateTime sDate, DateTime eDate, List<Record> listOfRecords, string ldccode)
        {
            ...
            exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                11, true, false, false);
            exRange.Value2 = "MobilePhone";
            col++;

            //CustomField1
            exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                11, true, false, false);
            exRange.Value2 = "CustomField1";
            col++;

            //CustomField2
            exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                11, true, false, false);
            exRange.Value2 = "CustomField2";
            col++;

            //CustomField3
            exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                11, true, false, false);
            exRange.Value2 = "CustomField3";
            col++;

            //CustomField4
            exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                11, true, false, false);
            exRange.Value2 = "CustomField4";
            col++;

            //CustomField5
            exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                11, true, false, false);
            exRange.Value2 = "CustomField5";
            col++;
            ...
            foreach (Record record in listOfRecords)
            {
                ...
                exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                    11, false, false, false);
                exRange.Value2 = record.BrandName;
                col++;
                ...
                exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                    11, false, false, false);
                exRange.Value2 = record.BillingType;
                col++;
                ...
                exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                    11, false, false, false);
                exRange.Value2 = string.IsNullOrEmpty(record.BillingFirstName) ? record.ServiceFirstName : record.BillingFirstName;
                col++;

                //BillingLastName
                exRange = RangeHelper.GetFormattedRange(ref exApp, new RangeColumn(ConvertColumn(col), row), new RangeColumn(ConvertColumn(col), row), Microsoft.Office.Interop.Excel.XlHAlign.xlHAlignCenter,
                    11, false, false, false);
                exRange.Value2 = string.IsNullOrEmpty(record.BillingLastName) ? record.ServiceLastName : record.BillingLastName;
                col++;
            ...
            exRange = (Excel.Range)exApp.get_Range("A1", "DG1");
        ...
        private static List<Record> GetListOfRecords(DateTime sDate, DateTime eDate, int premiseTypeId, int utilityId, int utilityTypeId, int salesChannelId, int vendorId, BrandObject brand)
        {
            ...
            //    m.SwitchDate, p.CreditCheck, m.CompanyName, b.BrandName
            ...
            try
            {
                using (SparkEntities entities = new SparkEntities())
                {
                    ...
                    //var myInClause = new int[] { 2, 5, 7, 8, 9 }; //2 Oasis data, 5 Provider data, 7 ENH Power, 8  Main Electricity, 9 Provider Power Mass

                    var idlist = brand.IDList.Select(x => x.id).ToList();

                    int[] myInClause = idlist.ToArray();
                    //var myInClause = brand.IDList.ToArray();
                    ...
                    var query = (from m in entities.Mains
                                 ...
                                 join usc in entities.UtilitySalesChannels on uty.UtilityId equals usc.UtilityId
                                 ...
                                 select new
                                 {
                                     ...
                                     BrandName = string.IsNullOrEmpty(b.BrandName) ? string.Empty : b.BrandName.ToUpper(),
                                     BrandId = p.BrandId,
                                     ProgramState = string.IsNullOrEmpty(p.State) ? string.Empty : p.State.ToUpper(),
                    ...
                    foreach (var item in query)
                    {
                        ...
                        if (item.PremiseTypeName == "Commercial")
                        {
                        ...
                        //URR for Maine Mass and New Hampshire Markets else it will be BillReady except for Provider which will be RateReady
                        string BillingType = string.Empty;
                        if (brand.Name.Contains("Provider"))
                        {
                            BillingType = "RateReady";//Change to be RateReady for all per zoho ticket #1399 Provider Batch File Updates

                        }
                        else
                        {
                            switch (item.ProgramState)
                            {
                                case "ME":
                                case "MA":
                                case "NH":
                        ...
                        if (brandName.Contains("ENERGY"))
                        {
                            brandName = brandName.Replace("ENERGY", string.Empty);
                        }
                        ...
                        Record record = new Record(item.Utility, item.CommodityType, utilityAccountNumber, item.PremiseTypeName,
                                                    ...
                                                    commercialCustomerType, item.CompanyName, brandName, BillingType, item.ProgramDescription, item.DefaultPricingPlanDescription);
                        records.Add(record);
                    }
                }
            }
        ...
        private static void SaveXlsDocument(ref string reportPath, ref string xlsFilename, ref string xlsFilePath, Excel.Workbook exBook, DateTime currentDate, string customerType, string market, string commodity, string salesChannel, string vendorName, BrandObject brand)
        {
            ...
            //Sold Date_ TPVVendorName_Brand_CustomerType_ Market&Commodity_SalesChannel_VendorName_CIS.xls
            //•	NIMO Electric Telesales:  2015_04_29_Calibrus_RES_NIMOE_TM_Immerge_CIS.xls
            //•	NIMO Gas Door to Door:  2015_04_29_Calibrus_RES_NIMOG_DTD_Empower_CIS.xls
            xlsFilename = String.Format("{0:yyyy_MM_dd}", currentDate) + "_" + brand.Name + "_Calibrus_" + customerType.ToUpper() + "_" + marketCommodity.ToUpper() + "_" + vendorSalesChannelAbbrev.ToUpper() + "_" + vendorName + "_CIS" + ".xls";
        ...
        //}
        private static void FTPFile(ref string reportPath, ref string xlsFilename, ref string xlsFilePath, DateTime currentDate, string customerType, string market, string commodity, string salesChannel, string vendorName, string HostName, string UserName, string Password, BrandObject brand)
        {
            string marketCommodity = market + commodity.Substring(0, 1); //take first letter of the commodity : (E)lectric/(G)as
        ...
        {
            Calibrus.ErrorHandler.Alerting alert = new Calibrus.ErrorHandler.Alerting("SparkProviderBatchXLSReport");
