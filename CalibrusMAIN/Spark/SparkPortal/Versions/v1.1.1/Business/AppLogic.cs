...
using System.Linq;
using Calibrus.SparkPortal.DataAccess;
...
namespace Calibrus.SparkPortal.Business
{
    public static class AppLogic
    {
        ...
        public static List<Utility> GetUtilities(string state)
        {
            ...
            {
                RateRepository repo = new RateRepository(ctx);
                List<Program> programs = repo.Filter(r => r.State == state && DateTime.Now > r.EffectiveStartDate && DateTime.Now < r.EffectiveEndDate, type => type.Utility);
        ...
        public static List<Utility> GetUtilities()
        {
            ...
            {
                RateRepository repo = new RateRepository(ctx);
                List<Program> programs = repo.Filter(r => DateTime.Now > r.EffectiveStartDate && DateTime.Now < r.EffectiveEndDate, type => type.Utility);
                foreach (Program p in programs)
                {
                    utilities.Add(p.Utility);
                }
        ...
        public static List<Program> GetPrograms(bool activeOnly, int id)
        {
            ...
            using (CustomSparkPortalDataEntities ctx = new CustomSparkPortalDataEntities())
            {
                RateRepository repo = new RateRepository(ctx);
                programs = activeOnly
                    ? repo.Filter(a => a.ProgramVendors.Any(v => v.VendorId == id) && a.EffectiveStartDate <= DateTime.Now && a.EffectiveEndDate > DateTime.Now, 
        ...
        public static List<Program> GetPrograms(int utilityId, int vendorId, string utilityType)
        {
            ...
            {
                RateRepository repo = new RateRepository(ctx);
                programs = repo.Filter(p => p.UtilityId == utilityId && p.ProgramVendors.Any(pv => pv.VendorId == vendorId) && p.UtilityType.UtilityTypeName.ToUpper() == utilityType.ToUpper() && p.EffectiveStartDate < DateTime.Now && p.EffectiveEndDate > DateTime.Now,
                        sort => sort.ProgramName, SortOrder.Ascending, u => u.UnitOfMeasure, p => p.PremiseType,
