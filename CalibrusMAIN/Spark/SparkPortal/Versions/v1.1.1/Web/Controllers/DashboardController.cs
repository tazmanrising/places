...
namespace Calibrus.SparkPortal.Web.Controllers
{
	...
	public class DashboardController : ApiController
	{
        ...
        [Route("api/verifiedaccountschart/{timeFrame:regex([dwmyDWMY]):length(1)}/{vendorId:int}/{officeId:int}")]
        [HttpGet]
        public IHttpActionResult GetVerifiedAccountsChartData(string timeFrame, int vendorId, int officeId)
        {
            DateTime sDate = GetStartDate(timeFrame.ToUpper());

            List<GetVerifiedAccountsChartSummary_Result> summary = Business.AppLogic.GetVerifiedAccountsChartData(sDate, vendorId, officeId);
            List<GetVerifiedAccountsChartDetail_Result> detail = Business.AppLogic.GetVerifiedAccountsChartDetailData(sDate, vendorId, officeId);
            List<Object> data = new List<Object> { summary, detail };

            return Ok(data);
        }

        [Route("api/topvendorschart/{timeFrame:regex([dwmyDWMY]):length(1)}")]
        [HttpGet]
        public IHttpActionResult GetTopVendorsChartData(string timeFrame)
        {
            DateTime sDate = GetStartDate(timeFrame.ToUpper());

            List<GetTopVendors_Result> summary = Business.AppLogic.GetTopVendorsData(sDate);
