...
angular.module('calibrus')
  .service('enrollmentService', function (userCache) {
    ...
    this.enrollmentToCalibrusRequest = function () {
      return {
        phone: order.authorizedParty.phone.replace(/[^\d]/gi, ''),
        phoneIsMobile: order.authorizedParty.phoneIsMobile,
        signature: order.signature && order.signature.dataUrl ? order.signature.dataUrl : null,
        contactPreference: {
          sms: order.contactPreference === 'sms',
          phone: order.contactPreference === 'phone',
          email: order.contactPreference === 'email'
        },
