...
using ClearviewScriptViewer.Models;


namespace ClearviewScriptViewer.Controllers
{
    public class ScriptLookupsController : ApiController
    {
        private ClearviewEntities db = new ClearviewEntities();

        // GET: api/ScriptLookups
        public IQueryable<ScriptName> GetScriptLookups()
        {
            //Returns list of scripts
            return db.ScriptLookups.Where(sl => sl.Active == true).Select(sl => new ScriptName { State = sl.State, SalesChannel = sl.SalesChannel, Script = sl.Script }).Distinct().OrderBy(x => x.State);
        }
