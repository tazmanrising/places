...
using System.Linq;
using Calibrus.ClearviewPortal.DataAccess;
...
namespace Calibrus.ClearviewPortal.Business
{
    public static class AppLogic
    {
        ...
        public static Main CreateRequest(DataAccess.Entities.Request request)
        {
            ...
            foreach (DataAccess.Entities.Request.RequestOrderDetail detail in request.OrderDetails)
            {
                ...
                OrderDetail o = new OrderDetail
                {
                    ...
                    ServiceBuildingType = detail.BuildingType,
        ...
        public static List<Utility> GetUtilities(string state)
        {
            ...
            {
                RateRepository repo = new RateRepository(ctx);
                List<Program> programs = repo.Filter(r => r.State == state, type => type.Utility);
        ...
        public static List<GetUtilityPrograms_Result> GetUtilityPrograms(int vendor, int office, string state, string zip)

        {
            List<GetUtilityPrograms_Result> result = null;
            using (CustomClearviewEntities ctx = new CustomClearviewEntities())
            {
                result = ctx.GetUtilityPrograms(userVendorId: vendor, userOfficeId: office, state: state, zip: zip).ToList();
            }
            return result;
        }
        ...
        public static User GetUser(int id)
        {
            ...
            {
                UserRepository repo = new UserRepository(ctx);
                user = repo.Find(x => x.UserId == id, type => type.UserLogs, type => type.UserType, type => type.Vendor, type => type.Office);
            }
            return user;
        }
        ...
        public static User GetUser(string userName, int? vendorId) // pass the vendor id :) stephen
        {
            User user;
            using (CustomClearviewEntities ctx = new CustomClearviewEntities())
            {
                UserRepository repo = new UserRepository(ctx);
                user = repo.Find(x => x.AgentId == userName && x.VendorId == vendorId, type => type.UserLogs, type => type.UserType);
            }
            return user;
        }
        ...
        public static List<User> GetUsers(bool activeOnly, bool getAdmins)
        {
            List<User> users;
            using (CustomClearviewEntities ctx = new CustomClearviewEntities())
            {
                ...
                if (!getAdmins)
                {
                    users = users.Where(x => x.UserType.SecurityLevel == 0).ToList();
                }
            }
        ...
        public static List<spGetAWSWavUrl_Result> GetAWSWavUrlByMainId(int mainId)
        {
            List<spGetAWSWavUrl_Result> result = null;
            using (CustomClearviewEntities ctx = new CustomClearviewEntities())
            {
                result = ctx.spGetAWSWavUrl(id: mainId, date: null, vendorNum: null).ToList();
            }
            return result;
        }

        public static List<spGetAWSWavUrl_Result> GetAWSWavUrlByDate(DateTime dateToGet)
        {
            List<spGetAWSWavUrl_Result> result = null;
            using (CustomClearviewEntities ctx = new CustomClearviewEntities())
            {
                result = ctx.spGetAWSWavUrl(id: null, date: dateToGet, vendorNum: null).ToList();
            }
            return result;
        }

        public static List<spGetAWSWavUrl_Result> GetAWSWavUrlByVendorNumber(string vendorNumber)
        {
            List<spGetAWSWavUrl_Result> result = null;
            using (CustomClearviewEntities ctx = new CustomClearviewEntities())
            {
                result = ctx.spGetAWSWavUrl(id: null, date: null, vendorNum: vendorNumber).ToList();
            }
            return result;
        }
