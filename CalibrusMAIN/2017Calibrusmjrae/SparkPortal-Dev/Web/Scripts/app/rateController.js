(function () {
    ...
    function rateController(portalData, $log, $location, $filter, $modal, $window) {
        ...
        vm.pageChanged = function () {
            ...
            vm.ratePage = vm.filteredRates.slice(start, end);
        }
        ...
        $log.info('TODAY: ' + thisDate.format());

        vm.filterRates = function () {
            vm.filteredRates = $filter('filter')(vm.rates, function (item, index) {
                var sDate = moment(item.EffectiveStartDate);
                var eDate = moment(item.EffectiveEndDate);
                if (item.ProgramName.toUpperCase().indexOf(vm.filter.toUpperCase()) > -1 ||
                    ...
                    return ((!vm.showInactive && thisDate <= eDate && thisDate >= sDate) ||
                        (vm.showInactive && (thisDate < sDate || thisDate > eDate)));
                }
                return false;
            });
